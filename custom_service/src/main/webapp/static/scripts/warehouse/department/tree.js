...
var tree={
    ...
    postUpdate:function(treeNode, newName){
        $.ajax({
                type : "post",
                url : "/warehouse/department/update",
                data : {
                    "id" : treeNode.id,
                    "name" : newName
                },
                ...
                success : function(data) {
                    if (data.result.code == "000003") {
                    $.alertError("您没有修改该层级的权限");
                    }
                    else if (data.result.code == "041004") {
                        $.alertError("当前层级不允许删除名称");
                    }
                    else if (data.result.code == "041002") {
                        $.alertError("修改的层级不存在");
                    }
                    else if (data.result.code == '041006') {
                        $.alertError('有未填写信息，请填写完整。');
                    }
                    else if (data.result.code == '041007') {
    ...
    postDelete:function(treeNode){
        $.ajax({
                type : "post",
                url : "/warehouse/department/delete",
                data : {
                    "id" : treeNode.id
                },
                dataType : "json",
                success : function(data) {
                    ...
                    else if (data.result.code == "041005") {
                        $.alertError("当前层级不允许删除");
                    }
                    else if (data.result.code == "041002") {
                        $.alertError("删除的层级不存在");
                    }
                    else if (data.result.code == '041008') {
    ...
    postAdd:function(treeNode){
        ...
        $.ajax({
                ...
                success : function(data) {
                    if (data.result.code == '000003') {
                        $.alertError('您没有当前层级下新增层级的权限');
                        tree.treeObj.removeNode(treeNode);
                        res = true;
                    }
                    else if (data.result.code == "041003") {
                        ...
                        tree.prependAddWarehouseBtn(null, treeNode);
                        res = true;
                    }
                },
                error : function(msg) {
    ...
    postExpand : function(treeNode) {
        $.ajax({
            type: 'post',
            url: '/warehouse/department/getTreeNodeList',
            async:false,
            data: {
    ...
    prependAddWarehouseBtn:function(treeId, treeNode) {
        if (treeNode.level == 1 && treeNode.id != undefined && treeNode.hasPermission) {
            var aId = "#" + treeNode.tId + "_a";
            var aObj = $(aId);
            var editStr = '<a class="fa fa-plus-square-o btn-addWarehouse" style="font-family:FontAwesome;" onclick="$.showCommonEditDialog(\'/warehouse/department/region/toWarehouseList?regionId='+treeNode.id+'\',\'仓库管理\',550,500);"></a>'
            aObj.append(editStr);
        }
    } 
...
setting = {
        ...
        view:{
            ...
            selectedMulti: false,
            addDiyDom:tree.prependAddWarehouseBtn
        }
    }
/**
 * 初始化页面
...
function init(rootPermission) {
    var rootNode = [{"id":0, "name":"物流", "isParent":true, "hasPermission":rootPermission}];
    tree.treeObj = $.fn.zTree.init($("#warehouseTree"), setting, rootNode);
}